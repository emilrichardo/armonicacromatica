  ...
  async function startListening(inputId = selectedInputId) {
    try {
      ...
      setDebugInfo({
        ...
        audioContextState: audioContext.state,
        trackReadyState: activeTrack?.readyState ?? 'missing',
        trackMuted: activeTrack?.muted ?? false,
        trackEnabled: activeTrack?.enabled ?? false,
        sampleRate: audioContext.sampleRate,
      })
      if (navigator.mediaDevices?.enumerateDevices) {
        const devices = await navigator.mediaDevices.enumerateDevices()
...
function HoleBubble({ note, slideNote, mode, tone, hole }) {
  ...
  return (
    <div className={bubbleClass}>
      ...
      {note.alt ? <span>{note.alt}</span> : <span>&nbsp;</span>}
